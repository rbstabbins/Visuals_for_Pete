// Macbeth ColorCheck  in prep for Lockman Spire Animation
//
// Import the MacBeth ColorChecker image, and segment
...
void setup(){
  ...
  fft.logAverages(15,3); // set the fft object to compute average bins, with a minimum bandwidth of 15Hz, with logarithmically increasing bandwidth, and including 3 channels per octave.
...
void draw(){
  ...
  for (int i = 0; i < n_bands ; i=i+1) {  // cycle through frequency bands
    ...
    float this_scale = map(a, 0, max_a, 0, 1) ;
    if ( this_scale > 1.0 ) { this_scale = 1; }
    if ( this_scale < 1.0 ) { this_scale = 0; }
    if ( this_scale > 1 ) { 
      println("Exceeded range");
      print(" a: "+a);
      print("    max_a: "+max_a);
      println("    scale: "+this_scale);
    }  
